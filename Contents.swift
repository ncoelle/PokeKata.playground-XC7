...
func findNearestPokemon(pokemons: [Pokemon]) -> (from: Point) -> (Pokemon, Double)? {
  let orderingFunc = { (lhs: (Pokemon, Double), rhs: (Pokemon, Double)) -> Bool in lhs.1 < rhs.1 }
  
  return { fromLocation in
      pokemons.filter { mon in mon.type == .Normal }
              .map { mon in (mon, mon.location) }
              .map { mon, pokeLoc in (mon, fromLocation.distance(to: pokeLoc)) }
              .minElement(orderingFunc)
  }
}
...
class PokeDistanceTests: XCTestCase {
  func testFindShortestDistanceToPokemon() {
    let sut = findNearestPokemon(mons) // put your SUT here
    XCTAssertEqualWithAccuracy(sut(from: playerLocation)!.1, 21.9, accuracy: 0.1)
  }
  
  func testFindNearestPokemonName() {
    let sut = findNearestPokemon(mons) // put your SUT here
    XCTAssertEqual(sut(from: playerLocation)!.0.name, "Mewtwo")
  }
}
